...
def logical_flowchart_starting(model,fitopts,task):
    ...
    if task == "Tf_loop_iteration":
        ...
        analysis.constant_temp.analyze_temperature_array(model,fitopts)
        ...
        analysis.constant_temp.check_completion(model,fitopts)
        ...
        analysis.constant_temp.analyze_temperature_array(model,fitopts,long=True)
        ...
        analysis.constant_temp.check_completion(model,fitopts,long=True)
...
def logical_flowchart_finished(model,fitopts,task):
    ...
    if task == "Tf_loop_iteration":
        ...
        flag = analysis.constant_temp.run_wham_heat_capacity(model,fitopts)
        if flag == 1:
            pass 
        else:
            simulation.constant_temp.folding_temperature_loop(model,fitopts)
            print "Starting Tf_loop_iteration"
            fitopts["last_completed_task"] = "Starting Tf_loop_iteration"
    elif task == "Tf_wham":
        print "Starting equilibrium simulations at Tf"
        simulation.constant_temp.run_equilibrium_simulations(model,fitopts)
        ...
        analysis.constant_temp.run_wham_heat_capacity(model,fitopts,long=True)
        ...
        parameter_fitting.prepare_newtons_method(model,fitopts,"ddG_MC2004")
        ...
        parameter_fitting.solve_newtons_method(model,fitopts,"ddG_MC2004")
        ...
        parameter_fitting.save_new_parameters(model,fitopts,"ddG_MC2004")
        fitopts["iteration"] += 1
