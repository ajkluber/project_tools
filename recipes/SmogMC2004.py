"""A recipe to reproduce Matysiak Clementi 2004 algorithm with the Smog AA model

Description:
------------

    This project manager is intended to automate the generation of a
enthalpically heterogeneous Go-model via the thermodynamic perturbation algorithm
...
def new_project(args):
    ...
    Models,Fittingopts = mdb.smog_AA_inputs.new_models(names)
...
#        os.chdir("..")
    ...
    for k in range(len(Models)):
        ...
        simulation.smog_AA_constant_temp.folding_temperature_loop(model,fitopts,iteration,new=True)
        fitopts["last_completed_task"] = "Starting: Tf_loop_iteration"
#        mdb.smog_AA_inputs.save_model(model,fitopts)
...
def continue_project(args):
    ...
    names = args.names
    Models,Fittingopts = mdb.smog_AA_inputs.load_models(names,dry_run=args.dry_run)
...
def logical_flowchart_finished(model,fitopts,task):
    ...
    if task == "Tf_loop_iteration":
        ...
        if flag == 1:
            ...
            simulation.smog_AA_constant_temp.folding_temperature_loop(model,fitopts,iteration)
            print "Starting Tf_loop_iteration"
            fitopts["last_completed_task"] = "Starting: Tf_loop_iteration"
    elif task == "Tf_wham":
        print "Starting equilibrium simulations at Tf"
        simulation.smog_AA_constant_temp.run_equilibrium_simulations(model,fitopts,iteration)
        ...
        simulation.smog_AA_constant_temp.start_next_Tf_loop_iteration(model,fitopts,iteration)
        fitopts["last_completed_task"] = "Starting: Tf_loop_iteration"
    else:
        raise ValueError("  Couldn't find next option for task: %s" % task)
...
def add_temperature_array(args):
    ...
    for i in range(len(Models)):
        ...
        if long == False:
            ...
            simulation.smog_AA_constant_temp.manually_add_temperature_array(model,fitopts,iteration,T_min,T_max,deltaT)
            Fittingopts[i]["last_completed_task"] = "Starting: Tf_loop_iteration"
        elif long == True:
            print "Manually adding equilibrium sims ", temps
            simulation.smog_AA_constant_temp.manually_add_equilibrium_runs(model,fitopts,iteration,temps)
            Fittingopts[i]["last_completed_task"] = "Starting: Equil_Tf"
        mdb.smog_AA_inputs.save_model(model,Fittingopts[i])
...
def extend_temperatures(args):
    ...
    for i in range(len(names)):
        ...
        fitopts = Fittingopts[i]
        iteration = fitopts["iteration"]
        simulation.smog_AA_constant_temp.manually_extend_temperatures(model,fitopts,iteration,method,temps,factor)
        mdb.smog_AA_inputs.save_model(model,Fittingopts[i])
