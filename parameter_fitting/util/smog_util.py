...
def get_rij_Vp(model):
    ...
    #assumes you are in the directory with traj.xtc and smog.pdb 
    time1 = time.time()
    traj = md.load("traj.xtc",top="smog.pdb")     # Loading from file takes most time.
    time2 = time.time()
    ...
    time1 = time.time()
    rij = md.compute_distances(traj,model.long_pairs-np.ones(model.long_pairs.shape),periodic=False)
    time2 = time.time()
    ...
    time1 = time.time()
    Vp = np.zeros((traj.n_frames,model.n_long_fitting_params),float)
    print "**********************************"
    print model.n_long_fitting_params
    print "**********************************"
    for i in range(model.n_long_fitting_params):   
        pair_idx = model.long_fitting_params[i]
...
#            pair_idx = model.model_param_interactions[param_idx][j]
        Vp[:,i] = Vp[:,i] + model.long_pair_V[pair_idx](rij[:,pair_idx])
    time2 = time.time()
...
def get_Vp_for_state(model,rij,state,n_frames):
    ...
    time1 = time.time()
    Vp_state = np.zeros((n_frames,model.n_long_fitting_params),float)
    for i in range(model.n_long_fitting_params):   
...
#        for j in range(len(model.model_param_interactions[param_idx])):
#            pair_idx = model.model_param_interactions[param_idx][j]
        Vp_state[:,i] = Vp_state[:,i] + model.long_pair_V[pair_idx](rij[state,pair_idx])
    time2 = time.time()
    print " Calculating Vp took: %.2f sec = %.2f min" % (time2-time1,(time2-time1)/60.)
