...
def get_dHk(model,rij,Fij_conts,Fij):
    ...
    for i in range(len(Fij_conts)): 
        ...
        dHk = dHk - Fij[i]*model.long_pair_eps[cont_idx]*model.long_pair_V[cont_idx](rij[:,cont_idx])
    return dHk
...
def get_Vp_plus_Vpk(model,Vp,rij,Fij_conts,Fij):
    ...
    for i in range(len(Fij_conts)):
        cont_idx = Fij_conts[i]
        param_idx = model.long_pairwise_param_assignment[cont_idx]
        Vp_plus_Vpk[:,param_idx] = Vp_plus_Vpk[:,param_idx] - \
                    Fij[i]*model.long_pair_V[cont_idx](rij[:,cont_idx])
    return Vp_plus_Vpk
...
def compute_Jacobian_for_directory(model,beta,mutants,Fij,Fij_pairs,bounds,state_labels,saveas="Q_phi.dat",test=False):
    ...
    Jacobian = np.zeros((2*len(mutants),model.n_long_fitting_params),float)
...
if __name__ == "__main__":
    ...
    name = "S6_2"
    model, fitopts = mdb.smog_AA_inputs.load_model(name)
    model.name = "S6"
